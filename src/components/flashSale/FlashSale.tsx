  ...
  return (
    <div className="px-[7%] mt-10 font-Poppins">
      <div className="relative rounded-lg overflow-hidden h-72">
          <img className=" w-full h-full object-cover -scale-x-100" src={`${products[randomNumber? randomNumber: 0]?.picture_urls[0]}`} alt="" />
          <div className="absolute w-full h-full top-0 rounded-lg overflow-hidden flex justify-end">
            <div className="w-2/5 h-full flex flex-col items-center justify-around py-10">
              <button type="button" className=' py-2 px-5 rounded-2xl bg-tertiary text-white text-[12px] cursor-pointer'>New {`${mapCategories(products[randomNumber? randomNumber: 0]?.category_id)}`}</button>
